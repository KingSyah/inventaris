document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add new DOM selectors for print panel
    const printReportTitle = document.getElementById('print-report-title');
    const printLocation = document.getElementById('print-location');
    const printInvDate = document.getElementById('print-inv-date');
    const printRepDate = document.getElementById('print-rep-date');
    ...
    // Function to update print panel
    const updatePrintPanel = () => {
        printReportTitle.textContent = reportTitleInput.value;
        printLocation.textContent = inventoryLocationInput.value || '-';
        
        try {
            printInvDate.textContent = inventoryDateInput.value ? new Date(inventoryDateInput.value).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-';
            printRepDate.textContent = reportDateInput.value ? new Date(reportDateInput.value).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-';
        } catch (e) {
            console.error("Invalid date format:", e);
            printInvDate.textContent = '-';
            printRepDate.textContent = '-';
        }
    };
    ...
    const renderSignatures = () => {
        ...
        // Check if the witness section is empty
        const witnessIsEmpty = !witness.name && !witness.title && !witness.id;
        
        // Hide the witness box completely if empty. The CSS will handle the rest.
        displayWitness.box.style.display = witnessIsEmpty ? 'none' : 'block';

        // Show/hide the entire signature section if all are empty
        const allEmpty = !responsible.name && !approver.name && !witness.name &&
                        !responsible.title && !approver.title && !witness.title &&
                        !responsible.id && !approver.id && !witness.id;
        
        signatureSection.style.display = allEmpty ? 'none' : 'flex';
    };

    const updateSignaturePrintClass = () => {
        if (toggleSignaturePrint.checked) {
            signatureSection.classList.remove('no-print');
            document.body.classList.add('print-with-signatures');
        } else {
            signatureSection.classList.add('no-print');
            document.body.classList.remove('print-with-signatures');
        }
    };
    ...
    const generateItemCode = (id) => {
        const prefix = state.reportDetails.prefix || 'BRG';
    ...
    itemConditionInput.addEventListener('focus', function() {
        // Clear the input when focused
        const currentValue = this.value;
        this.value = '';
        // Add a small delay before setting back the value
        setTimeout(() => {
            if (!this.value) {
